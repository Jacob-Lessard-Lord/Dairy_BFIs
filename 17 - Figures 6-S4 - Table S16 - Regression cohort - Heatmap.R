...
# Extraction r2 partiels entre les biomarqueurs et l'exposition d'intérêt ----

# FFQ #
r_partial_ffq <- bind_rows(results_ffq %>% 
                             select(Aliment, Identification, r_partial, p_value) %>%
                             filter(Aliment == "Milk" & Identification %in% c("δ-Valerobetaine", "Homostachydrine", "Milk multi-BFIs model")),
                           
                           results_ffq %>% 
                             select(Aliment, Identification, r_partial, p_value) %>%
                             filter(Aliment == "Butter" & Identification %in% c("PE (P-17:0/20:4, O-17:1/20:4)", "PC (P-17:0/20:4, O-17:1/20:4)", "Butter multi-BFIs model")),
                           
                           results_ffq %>% 
                             select(Aliment, Identification, r_partial, p_value) %>%
                             filter(Aliment %in% c("Cheese", "Cream", "Butter") &
                                      Identification %in% c("SM d18:1/17:0", "PC 17:0/18:1", "SM 43:2", "SM 43:1", "Cer d18:1/25:0", "CFF dairy multi-BFIs model"))) %>%
  rename("r_partial_ffq" = "r_partial",
         "p_value_ffq" = "p_value")

# Mean of 24h recalls #
r_partial_r24w_mean <- bind_rows(results_r24w_mean %>% 
                             select(Aliment, Identification, r_partial, p_value) %>%
                             filter(Aliment == "Milk" & Identification %in% c("δ-Valerobetaine", "Homostachydrine", "Milk multi-BFIs model")),
                           ...
                           results_r24w_mean %>% 
                             ...
                             filter(Aliment %in% c("Cheese", "Cream", "Butter") &
                                      Identification %in% c("SM d18:1/17:0", "PC 17:0/18:1", "SM 43:2", "SM 43:1", "Cer d18:1/25:0", "CFF dairy multi-BFIs model"))) %>%
  rename("r_partial_r24w_mean" = "r_partial",
         "p_value_r24w_mean" = "p_value")

# Day-of-draw 24h recall #
r_partial_r24w_q3 <- bind_rows(results_r24w_q3 %>% 
                             select(Aliment, Identification, r_partial, p_value) %>%
                             filter(Aliment == "Milk" & Identification %in% c("δ-Valerobetaine", "Homostachydrine", "Milk multi-BFIs model")),
                           ...
                           results_r24w_q3 %>% 
                             ...
                             filter(Aliment %in% c("Cheese", "Cream", "Butter") &
                                      Identification %in% c("SM d18:1/17:0", "PC 17:0/18:1", "SM 43:2", "SM 43:1", "Cer d18:1/25:0", "CFF dairy multi-BFIs model"))) %>%
  rename("r_partial_r24w_q3" = "r_partial",
         "p_value_r24w_q3" = "p_value")

# Merge des 3 tableaux
r_partial <- full_join(x = r_partial_ffq,
                       y = full_join(x = r_partial_r24w_mean,
                                     y = r_partial_r24w_q3,
                                     by = c("Aliment", "Identification")),
                       by = c("Aliment", "Identification"))

# Exportation des résultats
write.xlsx(r_partial, file = "Metabo/Table S15.xlsx")

rm(r_partial, r_partial_ffq, r_partial_r24w_mean, r_partial_r24w_q3)
